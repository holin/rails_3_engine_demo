...
* [edit Gemfile, add line: ] gem 'oauth' 
* [edit Gemfile, add line: ] gem 'oauth_china', :git => 'git://github.com/holin/oauth_china.git'
* [edit Gemfile, add line: ] gem 'cheese', :path => "../rails_3_engine_demo"
* rails generate cheese
* [examine config/initializers/cheese.rb to see basic config parameters]
* rake db:migrate (one of the migrations that you’ll see run came from the engine)
* [edit config/oauth/qq.yml]
* [edit config/oauth/sina.yml]

==== Usage

* visit http://localhost:3000/cheese/syncs/new?kind=qq
* visit http://localhost:3000/cheese/syncs/new?kind=sina
* Send Status
** token = Cheese::Token.find_by_kind("qq"); client = OauthChina::Qq.load(:access_token => token.token, :access_token_secret => token.secret);client.add_status(content)
** token = Cheese::Token.find_by_kind("sina"); client = OauthChina::Sina.load(:access_token => token.token, :access_token_secret => token.secret);client.add_status(content)
